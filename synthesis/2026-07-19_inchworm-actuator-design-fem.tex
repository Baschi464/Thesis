...
pressure, as shown in Figs.~\ref{fig:inchworm-fiberspacing}
and~\ref{fig:inchworm-wallthickness}. Furthermore, strategies to mitigate the
ballooning effect of the active internal chamber were investigated,
specifically by incorporating radial fiber reinforcements and by using an
elastomer with a higher elastic modulus (EcoFlex 00-50) for the internal
walls. The bending angles obtained for a single and for two simultaneously
pressurized chambers across these designs are compared in
Table~\ref{tab:inchworm-designcomparison}. Interestingly, suppressing the
ballooning effect did not reduce the three-chambered actuator's pressure
requirements, as the unpressurized chambers inherently act as a
strain-limiting layer during bending. The simulated stress distribution over
a single-chamber bending sequence is shown in Fig.~\ref{fig:inchworm-stress}.

\begin{figure}[tb]
 \centering
  \includegraphics[height=45mm]{figures/decomposed/comsolBend.png}
  \vspace*{-2mm}
  \caption{Simulated stress distribution of the optimized actuator over a
  single-chamber bending sequence of increasing pressure.}
  \label{fig:inchworm-stress}
\end{figure}

\begin{figure}[tb]
 \centering
  \includegraphics[width=0.82\columnwidth]{figures/decomposed/fiberspacing.png}
  \vspace*{-2mm}
  \caption{Effect of the circumferential fiber spacing on the bending angle
  when a single chamber is pressurized at 2~kPa.}
  \label{fig:inchworm-fiberspacing}
\end{figure}

\begin{figure}[tb]
 \centering
  \includegraphics[width=0.82\columnwidth]{figures/decomposed/wallthickness.png}
  \vspace*{-2mm}
  \caption{Effect of the silicone outer-wall thickness on the bending angle
  when a single chamber is pressurized at 3~kPa.}
  \label{fig:inchworm-wallthickness}
\end{figure}

\begin{table}[tb]
  \caption{Bending angle for one and two simultaneously pressurized chambers
  at 3~kPa (3~mm circumferential fiber spacing), compared across three
  actuator designs. The full FEM data are tabulated in the appendix
  (Tables~\ref{tab:app-wallthickness}--\ref{tab:app-designcomparison}).}
  \label{tab:inchworm-designcomparison}
  \centering
  \footnotesize
  ...
  \begin{tabular}{|l|c|c|}
    ...
    Baseline (EcoFlex 00-30) & 37.1 & 28.7 \\ \hline
    Dual-material (EcoFlex 00-50 walls) & 27.8 & 24.7 \\ \hline
    Radial fibers (3~mm) & 20.1 & 26.1 \\ \hline
  \end{tabular}
\end{table}
